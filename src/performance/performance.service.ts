  ...
  async createPerformance(user: User, data: any): Promise<Performance> {
    ...
    if (data.schedules && data.schedules.length > 0) {
      for (const scheduleData of data.schedules) {
        if (!scheduleData.date_time || !scheduleData.seats || scheduleData.seats.length === 0) {
          throw new BadRequestException('Invalid performance data. Please provide valid date_time and seats.');
        }
        ...
        for (const seatData of scheduleData.seats) {
          if (!seatData.seat_number) {
            throw new BadRequestException('Invalid seat data. Please provide a valid seat_number.');
          }
